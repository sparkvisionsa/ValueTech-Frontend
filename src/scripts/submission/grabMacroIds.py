...
#
async def update_report_with_macro_ids(
    report_id, macro_ids_with_pages, db_name="test", collection_name="reports"
):
    client = None
    try:
        ...
        if not existing_report:
            ...
            for coll_name in possible_collections:
                ...
                existing_report = await test_collection.find_one(
                    {"report_id": report_id}
                )
                if existing_report:
                    ...
                    print(
                        f"[MONGO_DB] Found report {report_id} in collection: {coll_name}",
                        file=sys.stderr,
                    )
                    break
        ...
        if not existing_report:
            print(
                f"[MONGO_DB] ERROR: Report with ID {report_id} not found in any collection",
                file=sys.stderr,
            )
            return False
        ...
        if not existing_assets:
            print(
                f"[MONGO_DB] ERROR: No asset_data found in report {report_id}",
                file=sys.stderr,
            )
            return False

        print(
            f"[MONGO_DB] Found {len(existing_assets)} existing assets in report",
            file=sys.stderr,
        )
        ...
        updated_assets = []
        for i, (asset, (macro_id, page_num)) in enumerate(
            zip(existing_assets, macro_ids_with_pages)
        ):
            ...
            print(
                f"[MONGO_DB] Updated asset {i}: id={macro_id}, pg_no={page_num}",
                file=sys.stderr,
            )
        ...
        if len(existing_assets) > len(macro_ids_with_pages):
            ...
            print(
                f"[MONGO_DB] Kept {len(remaining_assets)} additional assets unchanged",
                file=sys.stderr,
            )
        ...
        if update_result.modified_count > 0:
            print(
                f"[MONGO_DB] Successfully updated report {report_id} with {len(macro_ids_with_pages)} macro IDs and page numbers",
                file=sys.stderr,
            )
            return True
        else:
            print(
                f"[MONGO_DB] Report {report_id} found but no modifications made (possibly same data)",
                file=sys.stderr,
            )
            return True
...
async def update_report_pg_count(
    report_id, pg_count, db_name="test", collection_name="reports"
):
    """
    ...
    try:
        ...
        if not existing_report:
            ...
            for coll_name in possible_collections:
                ...
                if existing_report:
                    collection = test_collection
                    print(
                        f"[MONGO_DB] Found report {report_id} in collection: {coll_name} for pg_count update",
                        file=sys.stderr,
                    )
                    break
        ...
        if result.modified_count > 0:
            print(
                f"[MONGO_DB] Successfully set pg_count={pg_count} for report {report_id}",
                file=sys.stderr,
            )
        else:
            # Document may exist but field already equal to value; treat as success
            print(
                f"[MONGO_DB] pg_count update for report {report_id} completed (modified_count={result.modified_count})",
                file=sys.stderr,
            )
        return True
        ...
        print(
            f"[MONGO_DB] Error updating pg_count for report {report_id}: {e}",
            file=sys.stderr,
        )
        import traceback
...
async def retry_get_missing_macro_ids(
    browser, report_id, tabs_num=3, db_name="test", collection_name="reports"
):
    """
    ...
    try:
        ...
        if not report:
            print(
                f"[MONGO_DB] ERROR: Report with ID {report_id} not found",
                file=sys.stderr,
            )
            clear_process(process_id)
            return {
        ...
        await update_report_pg_count(
            report_id, total_pages, db_name=db_name, collection_name=collection_name
        )
        ...
        if all_macro_ids_with_pages:
            ...
            print(
                f"[MONGO_DB] Retry update for report {report_id}, modified_count={result.modified_count}",
                file=sys.stderr,
            )
